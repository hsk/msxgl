...
// バイト単位で0なら透明な画像描画
void sc3DrawSpriteNoClip(Sc3* sc3, Sc3* im, u8 x, u8 y) {
  u8 w = im->w;
  u8 h = im->h; 
  u8* src = im->data;
  u8* dt = sc3->data;
...
inline void sc3CopySpriteNoClip(Sc3* sc3, Sc3* im, u8 x, u8 y, i8 sx, i8 sy, u8 w, u8 h) {
  u8* sdt = im->data;
  u8* dt = sc3->data;
  sdt += im->h*sx + sy;
  i8 h1 = sc3->h;
...
inline void sc3CopySprite(Sc3* sc3, Sc3* im, i8 x, i8 y, i8 sx, i8 sy, i8 w, i8 h) {
  w += x;
  //  if (w < x) { i8 t = x; w=x; w=t; }
  if (w <= 0 || sc3->w <= x) return;
  if (x < 0) {sx-=x; x = 0;}
  ...
  if (h > sc3->h) h = sc3->h;
  h -= y;
  sc3CopySpriteNoClip(sc3,im,x,y,sx,sy,w,h);
}
...
void run4() {
  ...
  for (;;) {
    ...
    if (Keyboard_IsKeyPressed(KEY_RIGHT)) x++;
    if (Keyboard_IsKeyPressed(KEY_SPACE)) break;
    sc3FillRect(sc3,0,0,32,24,0);
    sc3FillRect(sc3,0,24,32,24,0x44);
    sc3CopySprite(sc3, (Sc3*)&ship2, x, y, s, 0, 4, 5);
    s = 4 - s;
    for (u8 i = 0; i < 8; i++) {
      u8 t = xs[i] = xs[i] - vs[i];
      sc3PutPixel(sc3, t >> 3, i * 3, 0xe);
